...
<body>
    ...
    include('elements/top_nav.php');

    if (isset($_SESSION['id_user']))
    {
        include 'config/connection.php';
        
        $sql    = "SELECT * FROM alamat WHERE id_user = '".$_SESSION['id_user']."'";
    ...
    <!-- search -->
    <div class="container-fluid mainContent fixed-top mt-3 pb-0" style="z-index: 2">
        <div class="form-inline row">
            <div class="input-group col-12">
                <input id="from" type="text" class="form-control" id="startInput" placeholder="Lokasi awal" list="start" />
                <datalist id="start">
                    <?php foreach ($alamat as $data) { ?>
                        <option value="<?= $data['alamat'] ?>"><?= $data['kategori'] ?></option>
                    <?php } ?>
                </datalist>
                <input id="to" type="text" class="form-control" id="finishInput" placeholder="Lokasi tujuan" list="finish" />
                <datalist id="finish">
                    <?php foreach ($alamat as $data) { ?>
                        <option value="<?= $data['alamat'] ?>"><?= $data['kategori'] ?></option>
                    <?php } ?>
